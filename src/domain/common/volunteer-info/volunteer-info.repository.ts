...
export class VolunteerInfoRepository extends Repository<VolunteerInfo> {
    ...
    async getAllUser(userIds: number[]) {
        const query = this.repository
            .createQueryBuilder('volunteerInfo')
            .select(['volunteerInfo.id', 'volunteerInfo.nationalNo'])
            .where('volunteerInfo.id in (' + userIds + ')');
    ...
    getAll(
        ...
        if (filterParam.volunteerInfoId)
            query.andWhere('volunteerInfo.id = :volunteerInfoId', { volunteerInfoId: filterParam.volunteerInfoId })

        if (filterParam.exceptVolunteerInfoIds?.length)
            query.andWhere('volunteerInfo.id not in (' + filterParam.exceptVolunteerInfoIds + ')')
