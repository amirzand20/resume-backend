...
import { FilterBaseInfoDto } from "@/domain/common/base-info/dto/filter-base-info.dto";
...
import { UpdateVolunteerInfoDto } from "@/domain/common/volunteer-info/dto/update-volunteer-info.dto";
...
export class VolunteerInfoService extends TypeOrmCrudService<VolunteerInfo> {
    ...
    constructor(
        ...
        private readonly exampPersonRepository: ExamPersonRepository,
    ...
    async getAll(
        ...
        if (selectedExamId) {
            const assignedUser = await this.exampPersonRepository.find({
                select: { volunteerInfoId: true },
                where: { examId: selectedExamId }
            });
            filterParam = { ...filterParam, exceptVolunteerInfoIds: assignedUser.filter(v => !!v.volunteerInfoId).map(v => v.volunteerInfoId) }
        }
